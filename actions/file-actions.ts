...
import { getRemoteFileStats, getRemoteFileContent } from "./ssh-actions";
...
3. Carefully restate all the tasks that the user asked you to complete, numbering each one.
...
5. Repeat for every item task you've identified
...
export async function downloadRemoteSelectedFiles(
  filePaths: string[],
  sshConfig: SSHConfig,
  rootDir?: string | null
): Promise<ActionState<{ content: string; filename: string }>> {
...
`;
    ...
    for (const filePath of filePaths) {
      try {
        ...
        console.log(`Reading remote file: ${fullPath} (original: ${filePath})`);

        // Use the getRemoteFileContent function to get the content
        const result = await getRemoteFileContent(
          sshConfig,
          filePath,
        ...
        if (result.isSuccess && result.data) {
          ...
          console.error(
            `Failed to read remote file ${filePath}:`,
            result.message
          );
          // Continue with other files even if one fails
        }
      } catch (error) {
        console.error(`Failed to read remote file ${filePath}:`, error);
        // Continue with other files even if one fails
      }
    }
    ...
    return {
      isSuccess: true,
      message: "Remote files combined successfully",
      data: {
        content: combinedContent,
        filename: "combined_remote_files.txt",
      },
    };
  } catch (error) {
    return {
      isSuccess: false,
      message: `Failed to combine remote files: ${(error as Error).message}`,
    };
  }
}
